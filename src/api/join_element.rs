...
impl<Packet: Sized> JoinElementLink<Packet> {
    pub fn new(input_streams: Vec<ElementStream<Packet>>, queue_capacity: usize) -> Self {
        ...
        for input_stream in input_streams {
            let (to_provider, from_consumer) = crossbeam_channel::bounded::<Option<Packet>>(queue_capacity);
            let (await_provider, wake_consumer) = crossbeam_channel::bounded::<task::Task>(number_consumers);

            let consumer = JoinElementConsumer::new(input_stream, to_provider.clone(), await_provider, wake_provider.clone());
        ...
        let provider = JoinElementProvider::new(from_consumers.clone(), await_consumers, wake_consumers, number_consumers);
        let overseer = JoinElementOverseer::new(from_consumers, overseer_wake_consumers, wake_provider);
...
mod tests {
    ...
    fn join_element() {
        ...
        let packet_generator0 = PacketIntervalGenerator::new(time::Duration::from_millis(100), packets.clone().into_iter());
        let packet_generator1 = PacketIntervalGenerator::new(time::Duration::from_millis(100), packets.clone().into_iter());
