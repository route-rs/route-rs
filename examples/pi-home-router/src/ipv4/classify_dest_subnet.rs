...
/// Classifies an Ipv4Packet by its destination subnet.
///
...
impl<SubnetName: Clone> Classifier for ByDestSubnet<SubnetName> {
    ...
    fn classify(&self, annotated_packet: &Self::Packet) -> Self::Class {
        for cidr in &self.subnets_by_len_desc {
            if cidr.contains(&annotated_packet.packet.dest_addr()) {
...
mod tests {
    ...
    fn match_inside_subnet() {
        let classifier = ByDestSubnet::new(
            maplit::hashmap! {
        ...
        packet.set_dest_addr(ip_inside_subnet);
    ...
    fn default_outside_subnet() {
        ...
        let mut packet = Ipv4Packet::empty();
        packet.set_dest_addr(ip_outside_subnet);
