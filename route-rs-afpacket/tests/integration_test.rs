...
use afpacket;
use rand::{self, Rng};
use route_rs_afpacket::{AfPacketInputLink, AfPacketOutputLink};
...
fn output_panics_when_built_without_ingressor() {
    ...
    AfPacketOutputLink::new().channel(s).build_link();
}
...
fn input_panics_when_built_without_channel() {
    AfPacketInputLink::new().build_link();
}
...
fn input_identity() {
    ...
    let results = rt.block_on(async {
        ...
        // set up link
        let link = AfPacketInputLink::new().channel(rx_rtr).build_link();
...
fn output_identity() {
    ...
    let pkt = rt.block_on(async {
        ...
        // set up link
        let link = AfPacketOutputLink::new()
